...
from ..models import GeoZone, Neighbourhood
import json

class GeoTransformer:
    """Transformer for geographic data (AGEB and Neighbourhood) to GeoJSON format."""
    ...
    def __init__(
        ...
        tourist_visitors_info: dict = None,
        # Type indicator
        geo_type: str = 'ageb',  # 'ageb' or 'neighbourhood'
    ):
        self.id = id
        ...
        self.neighbourhood_code = neighbourhood_code
        self.neighbourhood_name = neighbourhood_name
        # Tourist visitors info
        self.tourist_visitors_info = tourist_visitors_info
        # Type indicator
        self.geo_type = geo_type
        
    @classmethod
    def from_model(cls, model: Union[GeoZone, Neighbourhood]) -> 'GeoTransformer':
        """Create a GeoTransformer instance from a GeoZone or Neighbourhood model."""
        if isinstance(model, GeoZone):
            return cls._from_geozone(model)
        elif isinstance(model, Neighbourhood):
            return cls._from_neighbourhood(model)
        else:
            raise ValueError(f"Unsupported model type: {type(model)}")
    
    @classmethod
    def _from_geozone(cls, model: GeoZone) -> 'GeoTransformer':
        """Create a GeoTransformer instance from a GeoZone model."""
        return cls(
            id=model.id,
            cvegeo=model.CVEGEO,
    ...
    @classmethod
    def _from_neighbourhood(cls, model: Neighbourhood) -> 'GeoTransformer':
        """Create a GeoTransformer instance from a Neighbourhood model."""
        return cls(
            id=model.id,
            geometry=model.geometry,
            population=model.population,
            ...
            business_thefts_2019=model.business_thefts_2019,
            # Neighbourhood specific fields
            municipality_code=model.municipality_code,
            municipality_name=model.municipality_name,
            neighbourhood_code=model.neighbourhood_code,
            neighbourhood_name=model.neighbourhood_name,
            # Tourist visitors info
            tourist_visitors_info=model.tourist_visitors_info,
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # Add AGEB specific properties
        if self.geo_type == 'ageb':
            properties.update({
                'cvegeo': self.cvegeo,
        ...
        # Add Neighbourhood specific properties
        if self.geo_type == 'neighbourhood':
            properties.update({
                'municipality_code': self.municipality_code,
                'municipality_name': self.municipality_name,
                'neighbourhood_code': self.neighbourhood_code,
                'neighbourhood_name': self.neighbourhood_name,
                # Tourist visitors info
                'tourist_visitors_info': self.tourist_visitors_info,
            })
