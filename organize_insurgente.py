...
for feature in data['features']:
    ...
    # Check both SISTEMA and layer for Tren Interurbano
    is_insurgente = (props.get('SISTEMA') == 'INSURGENTE' or 
                    props.get('layer') == 'Tren Interurbano Primera Etapa')
        ...
        # If it's a line geometry (LineString)
            ...
            line_geometries[line_id] = {
                ...
                'properties': {
                    ...
                    'system': 'INSURGENTE',
                    'route': props.get('RUTA', ''),
                    'description': props.get('descriptio', ''),  # Include description if available
                    'layer': props.get('layer', '')
                },
                'geometry': feature['geometry']
            }
...
with open('geodjango/data/systems/organized/insurgente-stations.geojson', 'w') as f:
    json.dump(stations_geojson, f, indent=2, ensure_ascii=False)

with open('geodjango/data/systems/organized/insurgente-lines.geojson', 'w') as f:
    json.dump(lines_geojson, f, indent=2, ensure_ascii=False)

# Print some statistics
print(f"Found {len(lines['main']['stations'])} stations for the INSURGENTE system") 